...
- **θᵢ** = Joint angle (rotation about zᵢ₋₁ axis)
- **dᵢ** = Link offset (translation along zᵢ₋₁ axis)  
- **aᵢ** = Link length (distance between zᵢ₋₁ and zᵢ along xᵢ)
- **αᵢ** = Link twist (rotation about xᵢ axis)
...
<Tabs>
  <TabItem label="Frame Assignment Rules">
    ...
    **Systematic coordinate frame placement:**
    ...
    1. **Z-axis alignment:** zᵢ along joint i+1 axis
    2. **X-axis definition:** xᵢ = zᵢ₋₁ × zᵢ (common normal direction)  
    3. **Y-axis completion:** yᵢ = zᵢ × xᵢ (right-hand rule)
    4. **Origin placement:** At intersection of xᵢ and zᵢ axes
    
    **Special cases:**
    - Parallel axes: xᵢ direction arbitrary (choose convenient)
  ...
  <TabItem label="Transformation Matrix">
    ...
    <Card title="🔄 DH Transformation Matrix" icon="document">
    ...
    **Sequence of elementary transformations:**
    1. Rotate θᵢ about zᵢ₋₁
    2. Translate dᵢ along zᵢ₋₁  
    3. Translate aᵢ along xᵢ
    4. Rotate αᵢ about xᵢ
    </Card>
...
<Card title="⚙️ Elementary Transformation Types" icon="document">
**Pure translations:**
...
<Tabs>
  ...
  <TabItem label="Forward Kinematics">
    ...
    **Find:** Platform position $(x, y, z)$ and orientation $(α, β, γ)$
    
    **Nonlinear system:**
    $$f_i(x, y, z, α, β, γ) = |\mathbf{P}_i(x,y,z,α,β,γ) - \mathbf{B}_i|^2 - L_i^2 = 0$$
  ...
  <TabItem label="Inverse Kinematics">
    ...
    **Given:** Platform pose $(x, y, z, α, β, γ)$  
...
**Key Question:** How do we design and model a Stewart Platform that achieves ±1 μm positioning accuracy while maintaining structural stiffness for semiconductor wafer handling?
:::
...
- Positioning accuracy: **`±1 μm`** position, **`±0.01°`** orientation
...
- **`Stiffness requirement`**: >100 N/μm in all directions
...
2. **Moving platform attachment points:**
   ...
   Where ψ = 30° (rotated configuration for better isotropy)
...
4. **Workspace optimization:**
   ...
   **Condition number minimization** for isotropy:
   Adjust $R_b$, $R_p$, $h_0$ to minimize $\kappa = \sigma_{max}/\sigma_{min}$ of Jacobian
...
1. **Constraint equation setup:**
   ...
   Where:
   - $\mathbf{R}$ = Platform rotation matrix (function of α, β, γ)
...
2. **Newton-Raphson iteration:**
   ...
   **Jacobian matrix construction:**
   ...
   Where $\mathbf{q} = [x, y, z, α, β, γ]^T$ are platform coordinates
...
<CardGrid>
  ...
  <Card title="Precision Engineering" icon="star">
    ...
    **Accuracy:** ±1 μm positioning achieved  
    **Stiffness:** >100 N/μm structural performance  
