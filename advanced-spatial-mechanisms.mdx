...
$$R_{spherical}(α, β, γ) = R_z(γ) R_y(β) R_x(α)$$

**Alternative axis-angle form:**
$$R_{spherical}(\mathbf{k}, θ) = I + \sin θ [\mathbf{k}]_× + (1-\cos θ) [\mathbf{k}]_×^2$$
...
<Tabs>
  <TabItem label="Euler Parameterization">
    ...
    **Joint limits:**
    - α (roll): ±60° typical for biological joints  
    - β (pitch): ±90° depending on anatomy
    - γ (yaw): ±30° for finger metacarpal joints
    
    **Singularity:** When β = ±90°, α and γ rotations become indistinguishable
  ...
  <TabItem label="Quaternion Approach">
    ...
    **Constraint:** $|q|^2 = q_0^2 + q_1^2 + q_2^2 + q_3^2 = 1$
...
- **First axis rotation:** θ₁ about fixed axis
- **Second axis rotation:** θ₂ about perpendicular moving axis  
- **Coupling constraint:** Second axis orientation depends on first rotation

**Transformation sequence:**
$$T_{universal} = R_{z}(θ_1) R_{x'}(θ_2)$$
...
<Tabs>
  <TabItem label="Kinematic Analysis">
    ...
    **Input-output relationship:**
    ...
    For equal input angular velocities ω₁:
    $$ω_2 = ω_1 \frac{\cos θ_1}{1 - \sin^2 θ_1 \sin^2 α}$$
    
    Where α is the angle between shaft axes
...
1. **MCP joint workspace (spherical joint):**
   ...
   **Parameterization using Euler angles:**
   $$R_{MCP}(α, β, γ) = R_z(γ) R_y(β) R_x(α)$$
   
   **Joint limits:**
   - α (abduction/adduction): ±20°
   - β (flexion/extension): 0° to 90°  
   - γ (rotation): ±10°

2. **Reachable orientation set:**
   
   **Solid angle calculation:**
   $$\Omega = \int_{\alpha_{min}}^{\alpha_{max}} \int_{\beta_{min}}^{\beta_{max}} \int_{\gamma_{min}}^{\gamma_{max}} \sin β \, d\alpha \, d\beta \, d\gamma$$
...
3. **Fingertip workspace volume:**
   ...
   **Proximal phalanx contribution:**
   Sphere of radius L₁ = 45 mm around MCP center
...
4. **Workspace optimization:**
   ...
   **Condition number minimization:**
   Adjust joint angle distributions to minimize $\kappa = σ_{max}/σ_{min}$
...
1. **Individual finger singularities:**
   ...
   **Type 1 - Boundary singularities:**
   - Joint limits reached: θᵢ = θᵢ,max or θᵢ,min
...
3. **Singularity detection algorithms:**
   ...
   **Condition number monitoring:**
   $$\kappa_i = \frac{\sigma_{max}(J_i)}{\sigma_{min}(J_i)}$$
...
<Tabs>
  <TabItem label="6D Workspace Mapping">
    ...
    **Position-orientation workspace:**
    ...
    $$\Phi(x,y,z) = \{(α,β,γ) | \text{inverse kinematics solvable}\}$$
  ...
  <TabItem label="Dexterity Measures">
    ...
    **Condition number:** $\kappa = σ_{max}/σ_{min}$
    **Manipulability:** $m = \sqrt{\det(JJ^T)}$
    **Isotropy index:** $I = σ_{min}/σ_{max}$
